...
case class StageParallelSummary(stageId : Int, stageName : String, schedulerDelayRatio : Double, avgSchedulerDelayRatio:Double, avgSchedulerDelay:Double, avgDuration:Double, taskNum : Int)
    ...
    def getStageTaskParallelList(stageDataList: Seq[StageData]): Seq[StageParallelSummary] = {
      ...
      stageDataList.foreach(stageData => {
        if (stageData.status == StageStatus.COMPLETE) {
          ...
          StageParallelSummaryList = StageParallelSummaryList :+ new StageParallelSummary(stageData.stageId,stageData.name, ratio, avgSchedulerDelayRatio , avgSchedulerDelay,avgDuration,taskNum)
        }
       })
